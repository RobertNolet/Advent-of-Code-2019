...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 09:52:26 2019
...
class IntCode:
    def __init__(self, program, inp = iter([]), ip=0):
        self.mem = program
        self.ip = ip
        self.inp = iter(inp)
        self.rel = 0
...
def fromfile(filename, inp = iter([])):
    return IntCode(map(int, open(filename).readline().split(',')), inp)